...
def get_roles_assumable_by_external_accounts(profile_name, known_accounts):
                ...
                if isinstance(aws_account, list):
                    for account in aws_account:
                        ...
                        if current_account != accountn:  # Exclude the account itself
                            if not role_name in assumable_roles:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--profile", required=True, help="AWS profile to check.")
    parser.add_argument("-k", "--known-accounts", help="One or more comma separated AWS acoounds id that are known and you want to filter from the results.")
    args = parser.parse_args()
    
    profile_name = args.profile
    known_accounts = [a.lower() for a in args.known_accounts.strip().split(",")] if args.known_accounts else []
    current_account, assumable_roles = get_roles_assumable_by_external_accounts(profile_name, known_accounts)
    if assumable_roles:
        ...
        print("No roles found that can be assumed by external accounts.")
